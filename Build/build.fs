...
let initTargets () =
    let installDeps =
        BuildTask.create "InstallDependencies" [] {
            dotNetOptions <- DotNet.install (fun opts -> { opts with Channel = DotNet.CliChannel.LTS })

            DotNet.exec dotNetOptions "tool" "restore"
            |> printRes "Tool restore"
            ...
            DotNet.exec dotNetOptions "paket" paketCmd
            |> printRes "Paket install"

            DotNet.exec dotNetOptions "femto" $"{projDir} --resolve"
            |> printRes "Femto package resolution"
        }
    ...
    let build config =
        DotNet.build (fun opts -> { opts with Configuration = config }) (projDir @@ $"{projName}.fsproj")

    let buildDebug =
        BuildTask.create "BuildDebug" [ installDeps.IfNeeded; clean.IfNeeded ] { build DotNet.Debug }

    let buildRelease =
        BuildTask.create "BuildRelease" [ installDeps.IfNeeded; clean.IfNeeded ] { build DotNet.Release }
    ...
    let format =
        BuildTask.create "Format" [ installDeps.IfNeeded ] {
            DotNet.exec id "fantomas" $"{slnDir} -r"
            |> printRes "Fantomas"
        }
